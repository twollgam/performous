...
FretBoard::FretBoard(Game& game, Audio& audio, Song const& song, input::DevicePtr dev, GuitarStringProvider provider)
    ...
    m_background(findFile("fretboard.svg")), m_line(findFile("fretboard_dot.svg")), m_muted(findFile("muted_string.svg")),
    m_layout(parseLayout(findFile("fretboard.json"))) {

    m_menu.close();

    m_renderer.setRenderer(std::make_shared<SimpleGLRenderer>(game));
}
